...
class NowPlaying:
    ...
    def __init__(self, parent):
        ...
        self.t = threading.Thread(target=self.looper, daemon=True)
        self.t.start()
    ...
    def looper(self):
        ...
        while i < 70:
            frames.append(tkinter.PhotoImage(file='rotation.gif', format='gif -index %i' % i))
            if i == 0:
                self._image_canvas.configure(image=frames[i])
            i += 1
        while True:
            ...
            if self._pause_event.isSet():
                time.sleep(0.1)
                self._pause_event.clear()
            if sick_beetz.check_if_playing():
                if current_frame > frameCnt:
                    current_frame = 0
                self._image_canvas.configure(image=frames[current_frame])
                current_frame += 1
            if terminate.is_set():
                break
            time.sleep(0.05)
